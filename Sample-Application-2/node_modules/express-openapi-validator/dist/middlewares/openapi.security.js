...
function security(context, securityHandlers) {
    return async (req, res, next) => {
        ...
        if (!expressRoute) {
            return next(util_1.validationError(404, req.path, 'not found'));
        }
        ...
        if (!pathSchema) {
            ...
            return next(util_1.validationError(405, req.path, `${req.method} method not allowed`));
        }
        ...
        const securitySchemes = context.apiDoc.components && context.apiDoc.components.securitySchemes;
        if (!securitySchemes) {
            const message = `security referenced at path ${path}, but not defined in 'components.securitySchemes'`;
            return next(util_1.validationError(500, path, message));
        }
        try {
            ...
            const err = util_1.validationError(e.status, path, message);
            next(err);
        }
    };
}
    ...
    async executeHandlers(req) {
        ...
        const promises = this.securities.map(async (s) => {
            try {
                ...
                const handler = (this.securityHandlers && this.securityHandlers[securityKey]) ||
                    fallbackHandler;
